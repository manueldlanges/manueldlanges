...
@section('header')
	...
	@endphp
        ...
        <div class="ispost post-47213a4">
			@if(!empty($sentences))
					{{ @array_pop($sentences) }} {{ @array_pop($sentences) }} {{ @array_pop($sentences) }} <br>
						@foreach($related as $r)
							@if(data_exists($r))
          						<h2 style="font-weight:normal;"><a href="{{ image_url($r) }}">{{ $r }}</a></h2>
          					@endif
				@endforeach
		@endif
        </div>
...
@foreach($chunked2 as $keyword2)
		    		...
		    		<img src="{{ $image['url'] }}" alt="{{ $image['title'] }}" class="image" title="{{ ucwords($keyword) }}" width="" height="400px" onerror="this.onerror=null;this.src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQh_l3eQ5xwiPy07kGEXjmjgmBKBRB7H2mRxCGhv1tFWg5c_mWT';">
		  		</a>
@endforeach
@endforeach
		  	</div>
